...
import (
	...
	"github.com/opspilot/project-service/internal/config"
	"github.com/opspilot/project-service/internal/db"
	"github.com/opspilot/project-service/internal/server"
)
...
func main() {
	...
	projectv1.RegisterProjectServiceServer(grpcSrv, server.New(pool, logger))
...
func startHealthServer(port string, pool *pgxpool.Pool, logger *slog.Logger) {
	...
	mux.HandleFunc("/health", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, `{"status":%q,"service":"project-service"}`, s)
	})
